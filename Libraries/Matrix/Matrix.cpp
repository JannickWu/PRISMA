...
void MATRIX::setDouble(int num_int, int num_dec)
{

	int First_Number = num_int;    //First_Number=2
	int Second_Number = num_dec;   
...
void MATRIX::setInteger(int number)
{
	if (number < 10)
	{
		lightFirstNumber(Number[0]);
		lightSecondNumber(Number[number]);
	}
	else
	{
		int firstNumber = number / 10;
		int secondNumber = number % 10;
		lightFirstNumber(Number[firstNumber]);
		lightSecondNumber(Number[secondNumber]);
	}
}
...
{
	digitalWrite(LATCH, LOW);
	shiftOut(SER_COL, CLK_COL, MSBFIRST, B00000000);
	shiftOut(SER_ROW, CLK_ROW, MSBFIRST, B11111111);
	digitalWrite(LATCH, HIGH);
}
